...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	...
	if (sentence_ptr == NULL) //check for sentece NULL
	{
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;
		return NULL;
	}
	if (searchWord_ptr == NULL) //check for word NULL
	{
		*errorCode_ptr = ERROR_NULL_SEARCH_POINTER;
		return NULL;
	}
	if (!errorCode_ptr) //check for pointer NULL
	{
		return NULL;
	}

	int i = 0;	//loop for string
	int j = 0;	//iterrator for word
	int location = 0; //location for the start fo the word
	int counter = 0; //
	char * new_ptr = sentence_ptr;
	while (*(sentence_ptr + i) != '\0') //loop through till null character
	{
		if (*(sentence_ptr + i) == *(searchWord_ptr + j)) //if letter equals first letter in word continue
		{
			for (j = 0; j < strlen(searchWord_ptr); j++) //iterate through whole word
			{
				if (*(sentence_ptr + i + j) == *(searchWord_ptr + j))
				{
					location = i; //keep location as long as word continues to be true
					counter++; //counter is the length of the word
				}
			}
			if (counter != strlen(searchWord_ptr)) //resets incase parital word was present
			{
				j = 0;
				counter = 0;
			}
	...
	if (counter == 0 || (strlen(sentence_ptr) < strlen(searchWord_ptr))) // word not found or word larger taht search area
	{
		*errorCode_ptr = ERROR_SEARCH_NOT_FOUND;
		return NULL;
	}
	if (counter == strlen(searchWord_ptr)) //return for success
	{
		*errorCode_ptr = ERROR_CODE_SUCCESS;
		return (new_ptr + location);
	}
